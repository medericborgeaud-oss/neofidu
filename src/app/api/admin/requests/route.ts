...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isSupabaseConfigured()) {
      ...
      const paidTaxRequests = taxRequests.filter(req => req.status !== "pending");
      const transformedTaxRequests = paidTaxRequests.map(req => ({
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const isGenericRequest = requestType === "accounting" || requestType === "property" || id.startsWith("acc_") || id.startsWith("prop_");
    ...
    if (!updatedRequest && !updatedGenericRequest) {
      return NextResponse.json(
        { error: "Erreur lors de la mise à jour - demande non trouvée" },
        { status: 500 }
      );
    }
