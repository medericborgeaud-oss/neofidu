...
export async function findTaxRequestByReference(reference: string): Promise<TaxRequestDB | null> {
  ...
  if (error) {
    console.error("Erreur recherche demande fiscale Supabase:", error);
    return null;
  }
...
export async function findGenericRequestByReference(reference: string): Promise<GenericRequestDB | null> {
  ...
  if (error) {
    ...
    if (error.code !== 'PGRST116') {
      console.error("Erreur recherche demande générique Supabase:", error);
    }
    return null;
  }
...
export async function updateGenericRequestStatus(
  ...
  if (error) {
    console.error("Erreur mise à jour statut demande générique Supabase:", error);
    return null;
  }
